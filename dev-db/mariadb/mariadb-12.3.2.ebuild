...
src_prepare() {
	# 12.3.2 diverged from the 11.4.8 Gentoo patch tarball:
	# 0001 and 0003 need refresh; gcc-13/rocksdb and boost-1.89 fixes are upstream.
	eapply "${FILESDIR}"/${PN}-12.3.2-without-clientlibs.patch
	eapply "${WORKDIR}"/mariadb-patches/0002-libmariadb-cmake-find-GSSAPI-via-pkg-config.patch
	eapply "${FILESDIR}"/${PN}-12.3.2-find-package-gssapi.patch
	eapply "${WORKDIR}"/mariadb-patches/0004-cmake-don-t-install-mysql-d-.service-symlinks.patch
	eapply "${WORKDIR}"/mariadb-patches/0005-libmariadb-do-not-install-client-plugins.patch
		...
		for tmp_mysqld_password_source in mysql client ; do
			...
			if [[ -n "${MYSQL_ROOT_PASSWORD}" ]] ; then
				if [[ ${MYSQL_ROOT_PASSWORD} == *$'\n'* ]] ; then
					ewarn "Ignoring password from '${tmp_mysqld_password_source}' section due to newline"
					ewarn "character (do you have multiple password options set?)!"
					MYSQL_ROOT_PASSWORD=
					continue
				fi
