...
src_prepare() {
	...
	# The 'generator' executable only helps upstream regenerate the LSP
	# types from the metamodel. ::haskell disables it via a patch; do the
	# same here so it does not drag in extra dependencies.
	sed -i '/^executable generator$/a\  buildable: False' "${CABAL_PN}.cabal" || die
}
