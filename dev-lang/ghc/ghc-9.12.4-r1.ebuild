...
# Upstream regenerates hadrian-bootstrap-sources-${GHC_BINARY_PV}.tar.gz for
# every release, so the copies under ~ghc/9.12.1 .. ~ghc/9.12.4 differ while
# sharing one filename. Store it under a per-version name, as ghc-9.6.7 does,
# so each ebuild gets its own Manifest entry.
