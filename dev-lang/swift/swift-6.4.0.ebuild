...
SRC_URI="
	...
	https://github.com/apple/swift-asn1/archive/refs/tags/1.3.2.tar.gz -> swift-asn1-1.3.2.tar.gz
	...
	https://github.com/apple/swift-certificates/archive/refs/tags/1.10.1.tar.gz -> swift-certificates-1.10.1.tar.gz
	...
	https://github.com/apple/swift-nio/archive/refs/tags/2.92.2.tar.gz -> swift-nio-2.92.2.tar.gz
	...
	https://github.com/microsoft/mimalloc/archive/refs/tags/v3.0.3.tar.gz -> mimalloc-v3.0.3.tar.gz
	...
	https://github.com/swiftlang/swift-subprocess/archive/refs/tags/0.4.tar.gz -> swift-subprocess-0.4.tar.gz
	...
	https://github.com/swiftlang/swift-toolchain-sqlite/archive/refs/tags/1.0.9.tar.gz -> swift-toolchain-sqlite-1.0.9.tar.gz
	https://github.com/swiftlang/swift-tools-protocols/archive/refs/tags/${P}-RELEASE.tar.gz -> swift-tools-protocols-${PV}.tar.gz
...
BDEPEND="
	...
	>=virtual/zlib-1.3.1:=
	|| (
		>=dev-lang/swift-6.3.3
		~dev-lang/swift-5.10.1
	)
	$(llvm_gen_dep '
...
src_configure() {
	...
	if use libcxx; then
		...
		if [[ "${LLVM_SLOT}" -ge 22 ]]; then
			# While `__COUNTER__` is being standardized, Clang 22 considers it a
			# C2Y extension, and produces compilation errors by default. The
			# semantics haven't changed, so these can be downgraded to warnings.
		...
		# later are installed.
		#
		# This is required even when the system LLVM is 22 or later: the early
		# bootstrap components (`earlyswiftdriver` and its vendored llbuild
		# LLVM) are compiled by the Clang bundled with the host Swift toolchain,
		# which is always older than the system Clang. Swift 5.10.1 ships Clang
		# 15, which cannot parse GCC 16 headers.
		#
