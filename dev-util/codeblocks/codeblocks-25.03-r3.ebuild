...
SRC_URI="https://downloads.sourceforge.net/${PN}/${PN}-25.03.tar.xz"
S="${WORKDIR}/${PN}_25.03"
...
PATCHES=(
	"${FILESDIR}/${PN}-25.03_env.patch"
	"${FILESDIR}/${PN}-20.03_p13518_FortranProject-r378-autotools-build.patch"
	"${FILESDIR}/${PN}-25.03_fix_boost-1.89.0.patch"
)

src_prepare() {
	default

	# displayevents was copied from IncrementalSearch and still references
	# PLUGIN_WX_LIBS, which nothing defines. With --no-undefined the plugin
	# then fails to link every wx symbol. Other plugins use $(WX_LIBS).
	sed -i 's/$(PLUGIN_WX_LIBS)/$(WX_LIBS)/' \
		src/plugins/displayevents/Makefile.am || die

	# Let's make the autorevision work.
	echo "m4_define([SVN_REV], ${REV})" > revision.m4 || die
	echo "m4_define([SVN_DATE], ${REV_DATE})" >> revision.m4 || die
